...
public class ClickEventHandler implements EventHandler {
    ...
    public interface ClickHandler {
        void onCLick(Dom source, Event event);
    }

    private final ClickHandler handler; // 具体处理逻辑

    public ClickEventHandler(ClickHandler handler) {
    ...
    public void handle(Event event) {
        if (handler != null) {
            handler.onCLick(event.getSource(), event);
        }
    }
}
